...
bazel --output_user_root=$TMPDIR run //tools:heir-opt -- --mlir-to-secret-arithmetic="entry-function=func" --operation-balancer --annotate-secret-management --validate-noise --debug-only="ValidateNoise" --mlir-to-openfhe-bgv='entry-function=func ciphertext-degree=8' $PWD/func.mlir > func_output.mlir
