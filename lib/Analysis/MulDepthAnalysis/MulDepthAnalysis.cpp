...
LogicalResult MulDepthAnalysis::visitOperation(
  ...
  llvm::TypeSwitch<Operation &>(*op)
      .Case<openfhe::MulOp, openfhe::MulPlainOp, openfhe::MulConstOp,
            openfhe::MulNoRelinOp, arith::MulIOp, arith::MulFOp>(
          [&](auto mulOp) {
            // FIXME: what if openfhe/arith mixed dialect
            const MulDepthLattice *lhs, *rhs;
            llvm::TypeSwitch<Operation &>(*op)
                .template Case<openfhe::MulOp, openfhe::MulPlainOp,
                               openfhe::MulConstOp, openfhe::MulNoRelinOp>(
                    [&](auto _) {
                      lhs = operands[1];
                      rhs = operands[2];
                    })
                .template Case<arith::MulIOp, arith::MulFOp>([&](auto _) {
                  lhs = operands[0];
                  rhs = operands[1];
                });
            ...
            MulDepth lhsMulDepth = lhs->getValue();
            ...
            MulDepth rhsMulDepth = rhs->getValue();
            ...
            ChangeResult result =
                r->join(MulDepth::join(lhsMulDepth, rhsMulDepth));
