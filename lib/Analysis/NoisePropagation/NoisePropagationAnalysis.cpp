...
LogicalResult NoisePropagationAnalysis::visitOperation(
...
LogicalResult NoiseStatesAnalysis::visitOperation(
  ...
  //   LLVM_DEBUG(llvm::dbgs()
  //              << "Op " << noisePropagationOp->getName()
  //              << " with argument-dependent noise propagation encountered
  //              input"
  //                 " with unknown noise. Marking result noise as unknown.\n");
  ...
  //  // If the result is yielded, then it is assumed to be in a loop yield.
  //  Then
  //  // the best we can do is check to see if the op producing this value has
  //  // argument-independent noise. If so, we can propagate that noise.
  //  // Otherwise, we must assume the worst case scenario of unbounded noise.
