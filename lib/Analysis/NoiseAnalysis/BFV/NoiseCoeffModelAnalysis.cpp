  ...
  auto res = llvm::TypeSwitch<Operation &, LogicalResult>(*op)
                 ...
                 .template Case<arith::MulIOp>([&](auto mulOp) {
                   ...
                   NoiseState mult = NoiseModel::evalMul(
                       localParam, operandNoises[0], operandNoises[1]);
                 ...
                 .template Case<tensor::ExtractOp>([&](auto extractOp) {
                   ...
                   NoiseState extract = NoiseModel::evalMul(
                       localParam, operands[0]->getValue(), one);
                   // assume relinearize immediately after rotate
                   // when we support hoisting relinearize, we need to change
                   // this
                   NoiseState rotate =
...
template class NoiseAnalysis<bfv::NoiseByVarianceCoeffSkModel>;
