...
NoiseBySymbolCoeffModel::evalEncryptPk(const LocalParamType &param,
  ...
  auto ei_s_es_ui = Expression::add(ei_s, es_ui);
  return ei_s_es_ui;
}
...
typename NoiseBySymbolCoeffModel::StateType NoiseBySymbolCoeffModel::evalMul(
  ...
  auto k0 = Symbol(lhsSymbol.getSymbol());
  auto k1 = Symbol(rhsSymbol.getSymbol());
  // k0 * s * rhs
  Symbol s("s");
  Monomial k0_s = Monomial::multiply(k0, s);
  Monomial k1_s = Monomial::multiply(k1, s);
  Expression k0_s_rhs =
      Expression::multiply(Expression::multiply(k0_s, rhs), t);
  Expression k1_s_lhs =
      Expression::multiply(Expression::multiply(k1_s, lhs), t);
  // Expression k0_s_rhs =
  //     Expression::multiply(k0_s, rhs);
  // Expression k1_s_lhs =
  //     Expression::multiply(k1_s, lhs);
  return Expression::add(k0_s_rhs, k1_s_lhs);
}
